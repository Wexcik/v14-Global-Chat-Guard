const { Client, Intents,  EmbedBuilder, Collection, WebhookClient } = require('discord.js');
...
mongoose.connect('mongodb+srv://misumi:misumi@misumi.yrxjurc.mongodb.net/', {useNewUrlParser: true, useUnifiedTopology: true})
...
client.on('messageCreate', async message => {
  ...
  if (data.reklamBanKoruma && sonuc == "reklamKoruma") {
    ...
    if (Number(reklamBanKoruma[message.guild.id].get(message.author.id)) >= 5) {
      ...
      await client.sunucuLog(message.guild.id, {embeds: [new EmbedBuilder().setTitle(dil == "tr" ? "Reklam Koruma - Ban" : "Invite Filter - Ban").setDescription(dil == "tr" ? `${message.author} (\`${message.author.tag}\` - \`${message.author.id}\`) üyesi, ${message.channel} kanalında **reklam spamı** yaptığı için sunucudan **yasaklandı!**` : `${message.author} (\`${message.author.tag}\` - \`${message.author.id}\`) banned because of **invite spamming** in ${message.channel} channel!`).setFooter(client.user.username + " Invite Filter", client.user.avatarURL()).setTimestamp()]})
    };
  };
  ...
  if (dil.spamKoruma) {
    ...
    if (msjs.filter(m => (Date.now()-m.createdTimestamp) < 2000).size > 30) {
      ...
      await client.sunucuLog(message.guild.id, {embeds: [new EmbedBuilder().setTitle(dil == "tr" ? "Kanal Kilitlendi!" : "Channel Locked!").setDescription(dil == "tr" ? `${message.channel} kanalı spam yapıldığı için kilitlendi!` : `${message.channel} channel has been locked because of spamming!`).setFooter(client.user.username + " Spam Filter", client.user.avatarURL()).setTimestamp()]})
    };
  };
});
