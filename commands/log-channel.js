...
exports.run = async (client, message, args, prefix, dil, database) => {
  ...
  if (!kanal) return message.channel.send({embeds: [client.komutBilgi(this.help.name, dil).setAuthor({ name: `${message.member.displayName}`, iconURL: `${message.author.avatarURL({ dynamic: true, size: 2048 })}` }).setColor(message.member.displayHexColor)]})
