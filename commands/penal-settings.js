...
exports.run = async (client, message, args, prefix, dil, database) => {
  if (!args[0] || (args[0] !== "duration" && args[0] !== "limit" && args[0] !== "süre" && args[0] !== "sure") || !args[1] || isNaN(args[1])) return message.channel.send(
    {embeds: [client.komutBilgi(this.help.name, dil).setAuthor({ name: `${message.member.displayName}`, iconURL: `${message.author.avatarURL({ dynamic: true, size: 2048 })}` }).setColor(message.member.displayHexColor).addFields(
    { name: `${dil == "tr" ? "Not" : "Note"}`, value: `${dil == "tr" ? "Süre parametresi, susturulma süresidir (dakika).\nLimit parametresi ise kişinin belirlediğiniz limite ulaşınca cezalandırılacağı limittir." : "Duration parameter is penal duration of mute (minute).\nLimit parameter is the limit that the person will be punished upon reaching the limit you set."}`},
  )]})
