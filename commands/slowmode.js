...
exports.run = async (client, message, args, prefix, dil) => {
  if (!args[0] || isNaN(args[0]) || Number(args[0]) < 0 || Number(args[0]) > 60) return message.channel.send({embeds: [client.komutBilgi(this.help.name, dil).setAuthor({ name: `${message.member.displayName}`, iconURL: `${message.author.avatarURL({ dynamic: true, size: 2048 })}` }).setColor(message.member.displayHexColor)]})
