...
exports.run = async (client, message, args, prefix, dil) => {
  ...
  if (!args[0] || isNaN(args[0]) || Number(args[0]) < 1 || Number(args[0]) > 100) return message.channel.send({embeds: [client.komutBilgi(this.help.name, dil).setAuthor({ name: `${message.member.displayName}`, iconURL: `${message.author.avatarURL({ dynamic: true, size: 2048 })}` }).setColor(message.member.displayHexColor)]})
  ...
  message.channel.bulkDelete(Number(args[0])).then(async c => {
    ...
    await client.sunucuLog({embeds: [message.guild.id, new EmbedBuilder().setTitle(dil == "tr" ? "Sohbet Temizlendi!" : "Chat Cleared!").setDescription(dil == "tr" ? `${message.author}, ${message.channel} kanalında **${c.size}** mesaj sildi!` : `${message.author}, deleted **${c.size}** messages on ${message.channel}!`).setTimestamp().setColor(message.member.displayHexColor)]})
  });
};
