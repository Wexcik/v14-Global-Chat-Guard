...
exports.run = async (client, message, args, prefix, dil, database) => {
  ...
  if (!args[0]) return message.channel.send({embeds: [client.komutBilgi(this.help.name, dil)  .setAuthor({ name: `${message.member.displayName}`, iconURL: `${message.author.avatarURL({ dynamic: true, size: 2048 })}` }).setColor(message.member.displayHexColor)]})

  let girilen = args.join(' ');
  if (girilen === "list" || girilen === "liste") return message.channel.send({embeds: [new EmbedBuilder().setDescription(veri.length > 0 ? veri.join('\n') : (dil == "tr" ? "Bulunamadı!" : "Not found!")).setTitle(message.guild.name + dil == "tr" ? "Filtrelenmiş Kelimeler" : "Filtered Words")]})
