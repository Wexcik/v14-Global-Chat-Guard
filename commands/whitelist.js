...
exports.run = async (client, message, args, prefix, dil, database) => {
  ...
  if (veri.includes(hedef.id)) {
    ...
    let embd = new EmbedBuilder().setColor(message.member.displayHexColor).setDescription(dil == "tr" ? `${hedef} güvenli listeden kaldırıldı!` : `${hedef} has been removed from whitelist!`)
    message.channel.send({embeds: [embd]});
  } else {
    veri.push(hedef.id);
    await database.updateOne({ guildID: message.guild.id }, { guvenli: veri }, { upsert: true });
    let embd2 = new EmbedBuilder().setColor(message.member.displayHexColor).setDescription(dil == "tr" ? `${hedef} güvenli listeye eklendi! Tekrar aynı işlemi yaparak kaldırabilirsin.` : `${hedef} has been added to whitelist! If you want to remove it, you must type same text.`)
    message.channel.send({embeds: [embd2]});
  };
}; 
