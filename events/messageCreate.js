...
module.exports = async message => {
  ...
  if (cmd) {
    ...
    if(message.author.id !== client.ayar.sahip) {
      ...
      let clasEmbed = dil === "tr" ? new EmbedBuilder().setAuthor({ name: `${message.member.displayName}`, iconURL: `${message.author.avatarURL({ dynamic: true, size: 2048 })}` }).setTimestamp().setColor("RANDOM").addField(`__Bağlantılar__`, `**[Botu Ekle](${client.ayar.ekle})\n[Bota Oy Ver](${client.ayar.oyver})\n[DBL](${client.ayar.dblSayfa})\nDestek Sunucum: ${client.ayar.destekDavet}**`) : new EmbedBuilder().setAuthor({ name: `${message.member.displayName}`, iconURL: `${message.author.avatarURL({ dynamic: true, size: 2048 })}` }).setTimestamp().setColor("RANDOM").addField(`__Links__`, `**[Add Bot](${client.ayar.ekle})\n[Vote Bot](${client.ayar.oyver})\n[DBL](${client.ayar.dblSayfa})\nSupport Server: ${client.ayar.destekDavet}**`);
      if (!cmd.conf.enabled) return message.channel.send({embeds: [clasEmbed.setDescription(dil == "tr" ? `**\`${cmd.help.isim}\`  adlı komut şu anda kullanıma kapalıdır!**` : `**This command is currently not available!**`)]})
