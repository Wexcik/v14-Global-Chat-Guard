const { Client, EmbedBuilder, Collection, WebhookClient } = require('discord.js');
...
/* client.sayfaliEmbed = async function(description, author = false, footer = false, feautuers = false) {
    let menu= new EmbedBuilder().setTitle(`${client.user.username} - Komut menüsü`).setColor("#36393f").setThumbnail("https://cdn.discordapp.com/avatars/343496705196556288/50815afab8904313ca6a062e3dd7c88d.webp").setFooter(client.user.tag, client.user.avatarURL()).setDescription("*Yardım menüsü yükleniyor, lütfen bekleyiniz!*")
  ...
  client.komutBilgi = function(isim, dil) {
   ...
   // return knl.send(dil == "tr" ? `= Komut Bilgi = \nAdı :: ${komut.help.isim}\nAlternatif(ler) :: ${komut.conf.aliases.join(', ')}\nAçıklama :: ${komut.help.aciklama}\nKullanım :: ${komut.help.kullanim}` : `= Command Information = \nName :: ${komut.help.name}\nAliase(s) :: ${komut.conf.aliases.join(', ')}\nDescription :: ${komut.help.description}\nUsage :: ${komut.help.usage}`, { code: "asciidoc", split: true });
    return new EmbedBuilder().setDescription(`${dil == "tr" ? `**= Komut Bilgi =** \n\`Adı:\` ${komut.help.isim}\n\`Alternatif(ler):\` ${komut.conf.aliases.join(', ')}\n\`Açıklama:\` ${komut.help.aciklama}\n\`Kullanım:\` ${komut.help.kullanim}` : `**= Command Information =** \n\`Name:\` ${komut.help.name}\n\`Aliase(s):\` ${komut.conf.aliases.join(', ')}\n\`Description:\` ${komut.help.description}\n\`Usage:\` ${komut.help.usage}`}`);
    //return knl.send(new EmbedBuilder().setDescription(`\`\`\`asciidoc\n${dil == "tr" ? `= Komut Bilgi = \nAdı :: ${komut.help.isim}\nAlternatif(ler) :: ${komut.conf.aliases.join(', ')}\nAçıklama :: ${komut.help.aciklama}\nKullanım :: ${komut.help.kullanim}` : `= Command Information = \nName :: ${komut.help.name}\nAliase(s) :: ${komut.conf.aliases.join(', ')}\nDescription :: ${komut.help.description}\nUsage :: ${komut.help.usage}`}\`\`\``));
  };
  ...
  client.ihlalIslem = async function(message, sebep, puan) {
    ...
    if(!uyariMsj.has(message.channel.id)) {
      message.channel.send({embeds: [new EmbedBuilder().setColor(message.member.displayHexColor).setDescription(`${message.author} ${dil == "tr" ? `**${sebep.toLowerCase()}** içerikli mesaj yasak!` : `Message with **${sebep.toLowerCase()}** content is forbidden!`}`)]}).then(e => setTimeout(() => e.delete().catch(() => { }), 6000))
